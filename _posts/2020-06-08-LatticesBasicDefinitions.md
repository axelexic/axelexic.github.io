...
3. there exits a _fixed non-zero radius_ $\epsilon$, such that around
   every point $\vec{x}$ in the lattice, the (open) ball

   $$S_\epsilon(\vec{x}) := \left \lbrace y \in \RR^n \; :\; \norm{x -
   y} < \epsilon \right \rbrace$$

   contains no _other_ lattice points, i.e.,
...
computational purposes. For that, we will use the following
definition and later prove that the two are equivalent:

```Definition [Lattice with basis] {#lattice-definition}

A lattice $\L$ of _rank_ $m$ and _dimension_ $n$ is defined to
be the set of _all possible_ **integer linear combination** of $m$ _linearly independent_
...
For the basis-free definition of a lattice to be equivalent to the
basis-dependent definition, it's crucial that $\lbrace b_i\rbrace$s are
linearly independent over $\RR$ and not just $\ZZ$. To see why, consider
the set $\lbrace 1, \sqrt{2}\rbrace$. This set is linearly independent
over $\ZZ$ because $$\forall \alpha, \beta \in \ZZ:\quad \alpha +
\beta\cdot\sqrt{2} = 0 \iff \alpha = \beta = 0.$$

On the other hand, _for all_ $\epsilon > 0$, there exists $\gamma,
\delta \in \ZZ$ such that $$ 0 < |(\gamma + \delta\sqrt{2}) - (\alpha +
\beta\cdot\sqrt{2})| < \epsilon.$$

Therefore, even though $\lbrace 1, \sqrt{2}\rbrace$ is linearly
 independent over $\ZZ$, it does not generate a lattice in $\RR$ because
 $\lbrace 1, \sqrt{2}\rbrace$ is not linearly independent over $\RR$.
 (See [below](#zmodule-not-lattice) for a proof.)


```aside [$\ZZ[\sqrt{2\;}]$ is not discrete in $\RR$] {#zmodule-not-lattice}

Our goal is to show that for every $\epsilon > 0$, there exists $\gamma,
\delta \in \ZZ$ such that $$ 0 < |(\gamma + \delta\sqrt{2}) - (\alpha +
\beta\cdot\sqrt{2})| < \epsilon.$$ Without loss of generality, we assume
$0 < \epsilon < 1$. (If $\epsilon \ge 1$ then set $\epsilon$ to its
fractional value, i.e., $\epsilon \leftarrow \epsilon \;(\text{mod } 1)$
and adjust the value of $\gamma$ accordingly.)

It suffices to show that there exists $\eta, \nu \in \ZZ$ &mdash; not
both zero &mdash; such that $\eta + \sqrt{2}\nu < \epsilon$. Since
$\lbrace 1, \sqrt{2} \rbrace$ is _linearly dependent_ over $\RR$, there
exist infinitely many $x, y \in \RR$ such that $x + \sqrt{2} y = 0$.
Pick a positive integer $\nu$ and let $y = \nu$. Then $x = -\sqrt{2}\nu
\in \RR$ satisfies $x + \sqrt{2} y = 0$.

<span class="highlight">Claim</span>: For every $\epsilon$, there exists
$\eta, \nu \in \ZZ$ such that $\eta + \sqrt{2}\nu < \epsilon.$

Let $\eta = -\lceil{\sqrt{2}\nu}\rceil$, then $$\eta + \sqrt{2}\nu =
\sqrt{2}\nu - \lfloor \sqrt{2}\nu \rfloor.$$ Therefore, given
$\epsilon$, if there exists $\nu$ such that $\sqrt{2}\nu - \lfloor
\sqrt{2}\nu \rfloor < \epsilon$ then the claim is satisfied. Rearranging terms $$\sqrt{2} -
\frac{\lfloor \sqrt{2}\nu \rfloor}{\nu} < \frac{\epsilon}{\nu}$$ and
using [Dirichlet's approximation
theorem](https://en.wikipedia.org/wiki/Dirichlet%27s_approximation_theorem), such a $\nu$ always exists.
```
...
<span class="highlight">Rank</span>: The rank of $\L$ is the rank of **free $\ZZ$-module** generated by $\lbrace  \vec{b}_1, \cdots, \vec{b}_m \rbrace$. However, since $\L$ is a _free_ $\ZZ$-module, its rank is well defined and is independent of choice of basis. (<span class="highlight">Note</span>: This definition of rank does not assume that the set $\lbrace \vec{b}_i \rbrace$ is linearly independent.)

<span class="highlight">Full Rank Lattice</span>: A lattice is full rank if it's rank as a $\ZZ$-module is same as it's dimension, i.e., $m = n$ if the set $\lbrace \vec{b}_i \rbrace$  is independent over $\RR$.
...
For a given basis $\B$, the fundamental parallelepiped is defined as:
$$
  \P(\B) := \left \lbrace \B\cdot \vec{u}\;:\; \vec{u} \in \RR^m, \forall i: 0 \leq u_i < 1 \right \rbrace \subseteq \RR^n
$$
